...
def songs(request):
    songs = req.get(
        "http://songs-sn-labs-nzojihipolit.labs-prod-openshift-san-a45631dc5778dc6371c67d206ba9ae5c-0000.us-east.containers.appdomain.cloud/song"
        ).json()
...
def photos(request):
    photos = req.get(
        "https://pictures.1xhbh5of5kpi.us-south.codeengine.appdomain.cloud/picture"
        ).json()
...
def concerts(request):
    ...
    if user.is_authenticated:
        ...
        return render(request, "concerts.html", {"concerts": lst_of_concert})  #{insert code to render the `concerts.html` page with the data of {"concerts": lst_of_concert}}
    else:
        return HttpResponseRedirect(reverse("login"))
